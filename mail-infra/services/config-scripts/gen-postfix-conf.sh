...
# Accept mail only from authenticated clients
smtpd_recipient_restrictions = permit_sasl_authenticated, check_recipient_access hash:/etc/postfix/recipient_access, reject
...
# --- Generate recipient_access file ---
cat >"${CONFIGS_PATH}/recipient_access" <<EOF
# Recipient access control for ${MAIL_DOMAIN}
# This file blocks incoming emails to the specified domain
# Use 'postmap /etc/postfix/recipient_access' to compile this file
@${MAIL_DOMAIN}  REJECT This email address does not accept incoming messages
EOF

echo Postfix configuration successfully generated
echo "Recipient access file created for domain: ${MAIL_DOMAIN}"
